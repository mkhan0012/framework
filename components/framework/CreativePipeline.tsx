...
import { FramingVector, CustomVectorConfig } from "@/lib/types";
import ArtifactGenerator from "./ArtifactGenerator";
import PerceptionEngine from "./PerceptionEngine";

// --- CONFIGURATION ---
// UPDATED: Renamed phases to match the Trilogy Narrative (The Lab -> The Lens -> The Arena)
const PHASES = [
  { id: "INPUT", label: "INJECTION PORT", sub: "ENTRY POINT" },
  { id: "INFRASTRUCTURE", label: "THE LAB", sub: "BELIEF ENGINEERING" }, 
  { id: "PERCEPTION", label: "THE LENS", sub: "TRUTH IS OPTIONAL" },    
  { id: "CONFLICT", label: "THE ARENA", sub: "ARGUELY" },               
  { id: "OUTCOME", label: "FINAL ANALYSIS", sub: "SURVIVOR VARIANT" }
];

const DEFAULT_VECTORS: FramingVector[] = ["FEAR", "OUTRAGE", "VALIDATION", "CONFUSION"];
...
// Theme Map
const BASE_VECTOR_THEMES: Record<string, { color: string, glow: string, border: string, hex: string }> = {
...
export default function CreativePipeline({ onExit }: { onExit: () => void }) {
  ...
  // Custom Vector State
  const [customVectors, setCustomVectors] = useState<CustomVectorConfig[]>([]);
  ...
  // Derive Theme
  const getVectorTheme = (v: string) => {
      if (BASE_VECTOR_THEMES[v]) return BASE_VECTOR_THEMES[v];
      const custom = customVectors.find(cv => cv.label === v);
      if (custom) return { color: custom.color, glow: custom.glow, border: custom.border, hex: custom.hex };
      return BASE_VECTOR_THEMES["FEAR"];
  };

  const theme = getVectorTheme(vector as string);
  ...
  const toggleListening = () => {
    ...
    if (!SpeechRecognition) {
        addLog("AUDIO HARDWARE NOT DETECTED.");
        return;
    }

    setIsListening(true);
    addLog("AUDIO INJECTION PORT OPEN...");
  ...
  const handleAddCustomVector = () => {
      if (!newVectorName) return;
      const newVec: CustomVectorConfig = {
          label: newVectorName.toUpperCase(),
          color: "text-pink-500",
          glow: "bg-pink-500",
          border: "border-pink-500",
          hex: "#ec4899"
      };
      setCustomVectors([...customVectors, newVec]);
      setVector(newVec.label);
      setShowCustomUI(false);
      setNewVectorName("");
      addLog(`NEW PROTOCOL ADDED: ${newVec.label}`);
  };
      ...
      const animate = () => {
          ...
          if (isListening) {
             ...
             for (let i = 0; i < canvas.width; i++) {
                 const y = canvas.height / 2 + Math.sin(i * 0.05 + Date.now() * 0.01) * (Math.random() * 50);
                 ctx.lineTo(i, y);
             }
             ctx.stroke();
          } else {
  ...
  const playSound = (type: "drone" | "glitch" | "alert" | "click" | "success" | "print" | "modem") => {
    ...
    if (type === "click") { 
        osc.type = "square"; osc.frequency.setValueAtTime(800, now); gain.gain.setValueAtTime(0.02, now); osc.start(now); osc.stop(now + 0.05); 
    } else if (type === "success") { 
        osc.type = "sine"; osc.frequency.setValueAtTime(600, now); osc.frequency.exponentialRampToValueAtTime(1200, now + 0.2); gain.gain.setValueAtTime(0.05, now); gain.gain.exponentialRampToValueAtTime(0.001, now + 0.5); osc.start(now); osc.stop(now + 0.5); 
    } else if (type === "glitch") {
        osc.type = "sawtooth"; osc.frequency.setValueAtTime(Math.random() * 500 + 100, now); gain.gain.setValueAtTime(0.1, now); osc.start(now); osc.stop(now + 0.1);
    } else if (type === "drone") {
        osc.type = "triangle"; osc.frequency.setValueAtTime(50, now); gain.gain.setValueAtTime(0.05, now); gain.gain.exponentialRampToValueAtTime(0.001, now + 2); osc.start(now); osc.stop(now + 2);
    } else if (type === "print") {
        osc.type = "square"; osc.frequency.setValueAtTime(800, now); gain.gain.setValueAtTime(0.05, now); osc.start(now); osc.stop(now + 0.1);
    } else if (type === "modem") {
        // Handshake sound
        osc.type = "sawtooth"; 
  ...
  const triggerSubliminal = () => { const word = SUBLIMINALS[Math.floor(Math.random() * SUBLIMINALS.length)]; setSubliminal(word); setTimeout(() => setSubliminal(null), 60); };
  ...
  const runSimulation = async () => {
    ...
    for (let i = 1; i <= generations; i++) {
        setCurrentGen(i);
        setPhase("INFRASTRUCTURE"); addLog(`GEN ${i}: INGESTING DATA [VECTOR: ${vector}]...`); if (Math.random() > 0.5) triggerSubliminal(); await new Promise(r => setTimeout(r, 2000));
    ...
    setHistory(prev => [{ id: Date.now(), input: inputText, output: currentText, vector: vector as string, timestamp: new Date().toLocaleTimeString() }, ...prev]);
    addLog("SIMULATION COMPLETE.");
  };

  // --- ACTIONS ---
  const handleReceipt = async () => { if (receiptOpen) { setReceiptOpen(false); return; } setReceiptOpen(true); playSound("print"); if (!analysisText) { setIsAnalyzing(true); const result = await generateDeepAnalysis(processedText); setAnalysisText(result); setIsAnalyzing(false); } };
  ...
  // --- UPDATED HANDLE BRIDGE: POINTS TO SIMULATOR ---
  const handleBridge = () => {
      setIsTransferring(true);
      addLog("INITIATING HANDSHAKE PROTOCOL...");
      playSound("modem");

      // 1. Construct the Viral Payload
      const payload = {
          origin: "FRAMEWORK_CORE",
          text: processedText,   // The mutated survivor text
          vector: vector,        // FEAR, OUTRAGE, etc.
          virality: virality,    // 0-100 score
          entropy: entropy       // Chaos level
      };

      // 2. Encode for transport
      const encoded = encodePayload(payload);
      
      // 3. Execute Transfer to SIMULATOR
      setTimeout(() => {
          // POINT DIRECTLY TO SIMULATOR PAGE
          const baseUrl = "https://truthis-optional.vercel.app/simulator"; 
          // Send the payload via query param called 'signal'
          window.open(`${baseUrl}?signal=${encoded}`, "_blank");
  ...
  return (
    <div className={`fixed inset-0 bg-theme-bg text-theme-text font-mono overflow-hidden select-none ${crtMode ? "crt-effect" : ""}`}>
      ...
      <header className="absolute top-0 w-full p-4 md:p-6 flex justify-between items-start z-40 border-b border-theme-border bg-theme-bg/50 backdrop-blur-sm">
        ...
        <div className="flex flex-wrap gap-4 items-center justify-end">
            <button onClick={() => setCrtMode(!crtMode)} className={`text-[10px] uppercase border px-3 py-1 rounded-full transition-all border-theme-border hover:border-theme-primary`}>CRT: {crtMode ? "ON" : "OFF"}</button>
            <button onClick={() => setShowGuide(!showGuide)} className={`text-[10px] uppercase border px-3 py-1 rounded-full transition-all border-theme-border hover:border-theme-primary`}>GUIDE: {showGuide ? "ON" : "OFF"}</button>
      ...
      <main className="absolute inset-0 flex items-center justify-center pt-16 pb-12">
        <div className="absolute w-full h-[1px] bg-theme-border"></div>
        ...
        {phase === "INPUT" && (
            <div className="relative z-10 flex flex-col items-center justify-center gap-6 animate-in fade-in zoom-in duration-500 w-full max-w-2xl p-4 h-full overflow-y-auto">
                ...
                <div className="w-full flex gap-2 justify-center flex-wrap mb-4">
                    {PRESETS.map((preset, i) => <button key={i} onClick={() => setInputText(preset)} className="text-[9px] border border-theme-border px-3 py-2 rounded-lg hover:bg-theme-border hover:text-white transition-colors uppercase tracking-wider">Preset {i + 1}</button>)}
                </div>

                {/* MAIN INPUT WITH AUDIO PORT */}
                <div className="relative w-full">
                    <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="TYPE BELIEF HERE..." className={`bg-transparent text-center text-lg md:text-3xl text-white outline-none placeholder-zinc-800 font-bold uppercase tracking-widest w-full border-b border-theme-border pb-4 focus:${theme.border} transition-colors`} autoFocus />
                    <button onClick={toggleListening} className={`absolute right-2 bottom-4 text-xs ${isListening ? "text-red-500 animate-pulse" : "text-zinc-600 hover:text-white"}`}>
                        {isListening ? "● LISTENING" : "🎤 AUDIO PORT"}
                    </button>
                </div>

                {inputText && (
                    <div className="grid grid-cols-2 gap-4 w-full text-[10px] font-mono border-b border-theme-border pb-8">
                        <div><div className="flex justify-between mb-1"><span>COMPLEXITY</span> <span>{Math.round(complexity)}%</span></div><div className="w-full h-1 bg-theme-border rounded"><div className="h-full bg-zinc-500 rounded" style={{width: `${complexity}%`}}></div></div></div>
                        <div><div className="flex justify-between mb-1"><span>VIRALITY</span> <span>{Math.round(virality)}%</span></div><div className={`w-full h-1 bg-theme-border rounded`}><div className={`h-full ${theme.glow.replace("/10", "")} rounded`} style={{width: `${virality}%`}}></div></div></div>
                    </div>
                )}
                ...
                <div className="w-full">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full">
                        <div className="flex flex-col gap-2">
                            <label className="text-[9px] uppercase text-zinc-600">Framing Vector</label>
                            <div className="flex flex-wrap gap-2">
                                {DEFAULT_VECTORS.map(v => (<button key={v} onClick={() => setVector(v)} className={`text-[9px] border px-2 py-2 rounded transition-all flex-1 ${vector === v ? `${theme.border} ${theme.color} ${theme.glow}` : "border-theme-border text-zinc-600 hover:border-zinc-600"}`}>{v}</button>))}
                                {customVectors.map(v => (<button key={v.label} onClick={() => setVector(v.label)} className={`text-[9px] border px-2 py-2 rounded transition-all flex-1 ${vector === v.label ? `${theme.border} ${theme.color} ${theme.glow}` : "border-theme-border text-zinc-600 hover:border-zinc-600"}`}>{v.label}</button>))}
                                <button onClick={() => setShowCustomUI(true)} className="text-[9px] border border-dashed border-zinc-700 px-2 text-zinc-500 hover:text-white hover:border-white">+</button>
                            </div>
                            
                            {/* CUSTOM VECTOR UI */}
                            {showCustomUI && (
                                <div className="mt-2 p-3 border border-zinc-800 bg-zinc-900 animate-in fade-in slide-in-from-top-1">
                                    <input type="text" placeholder="PROTOCOL NAME (e.g. HOPE)" value={newVectorName} onChange={(e) => setNewVectorName(e.target.value)} className="w-full bg-black border border-zinc-700 p-2 text-[10px] text-white uppercase mb-2" />
                                    <button onClick={handleAddCustomVector} className="w-full bg-zinc-800 text-white text-[9px] py-1 uppercase hover:bg-zinc-700">Initialize Custom Protocol</button>
                                </div>
                            )}
                        </div>
                ...
                {history.length > 0 && (
                    <div className="w-full mt-12 pt-8 border-t border-theme-border">
                        ...
                        <div className="space-y-2 max-h-32 overflow-y-auto pr-2 custom-scrollbar">{history.map((item) => (<div key={item.id} className="flex justify-between items-center text-[10px] border border-theme-border p-2 rounded hover:border-zinc-700 cursor-pointer bg-black/50" onClick={() => { setProcessedText(item.output); setInputText(item.input); setPhase("OUTCOME"); }}><span className="text-zinc-500 truncate w-1/3">{item.input}</span><span className="text-zinc-700">&rarr;</span><span className="text-zinc-300 truncate w-1/3">{item.output}</span><span className={getVectorTheme(item.vector).color}>{item.vector}</span></div>))}</div>
                    </div>
                )}
            </div>
        )}

        {/* ... (Keep INFRASTRUCTURE / PERCEPTION / CONFLICT rendering same as original, simplified here for space, assume unchanged logic just using theme variables) ... */}
        ...
        {phase === "OUTCOME" && (
            <div className="relative z-10 w-full max-w-5xl flex flex-col items-center justify-center p-4">
                ...
                <div className="mt-12 flex gap-4 flex-wrap justify-center">
                    <button onClick={handleBridge} disabled={isTransferring} className={`text-xs text-purple-400 hover:text-white uppercase tracking-widest border border-purple-900 hover:bg-purple-900 px-4 py-2 rounded flex items-center gap-2 transition-all ${isTransferring ? "opacity-50 cursor-wait" : ""}`}>
                        {isTransferring ? (<><span className="animate-spin">⟳</span> UPLOADING...</>) : (<><span>⚡</span> TEST IN PERCEPTION LAYER</>)}
                    </button>

                    <button onClick={() => setShowPerception(true)} className="text-xs text-cyan-500 hover:text-white uppercase tracking-widest border border-cyan-900 hover:bg-cyan-900 px-4 py-2 rounded">PREVIEW PERCEPTION</button>
        ...
        {/* --- OVERLAYS --- */}
        {showArtifact && <ArtifactGenerator text={processedText} vector={vector as any} onClose={() => setShowArtifact(false)} />}
        {showPerception && <PerceptionEngine text={processedText} onClose={() => setShowPerception(false)} />}
      </main>

      <footer className="absolute bottom-0 w-full border-t border-theme-border bg-theme-bg p-2 overflow-hidden">
